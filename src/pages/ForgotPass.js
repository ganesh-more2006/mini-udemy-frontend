...
const ForgotPass = () => {
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.message || "OTP is Expire!");
        }
    };
...
const styles = {
    container: { height: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
    glassCard: { background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(10px)', padding: '40px', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.1)', textAlign: 'center', width: '400px' },
    title: { color: '#fff', marginBottom: '10px' },
    subtitle: { color: '#94a3b8', marginBottom: '25px', fontSize: '0.9rem' },
    input: { width: '100%', padding: '12px', borderRadius: '8px', border: 'none', background: 'rgba(255,255,255,0.1)', color: '#fff', marginBottom: '15px', boxSizing: 'border-box' },
    btn: { width: '100%', padding: '12px', borderRadius: '8px', background: '#4f46e5', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 'bold' }
};
